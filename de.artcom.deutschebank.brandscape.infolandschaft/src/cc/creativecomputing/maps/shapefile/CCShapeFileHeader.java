...
public class CCShapeFileHeader {
	 ...
	 * @param file
	 */
	public CCShapeFileHeader(CCShapeFile parent_shapefile, File file) {
	...
	public void read(ByteBuffer bb) {
		...
		if (SHP_MAGIC_read != SHP_MAGIC) {
			throw new CCShapeFileException("(ShapeFile) error: SHP_MAGIC = " + SHP_MAGIC + ", File: " + file);
		}
		// file length
		_myFileLength = bb.getInt(24);
		bb.order(ByteOrder.LITTLE_ENDIAN);
		int SHP_version_read = bb.getInt(28);
		if (SHP_version_read != SHP_VERSION) {
			throw new CCShapeFileException("(ShapeFile) error: SHP_VERSION = " + SHP_VERSION + ", File: " + file);
		}

		_myShapeType = bb.getInt(32);

		try {
			shape_type = CCShpShapeType.byID(_myShapeType);
		} catch (Exception e) {
			e.printStackTrace();
		}
