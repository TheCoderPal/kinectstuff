 ...
 * 
 */
public class ILUserThrowDemo extends CCApp {
	...
	private Map<Integer, ILThrowChecker> _myUserMap = new HashMap<Integer, ILThrowChecker>();

	public void setup() {
		addControls("app", "app", this);
		addControls("app", "throw", ILThrowChecker.class);
...
//		g.perspective(95, width / (float) height, 10, 150000);
	...
	private void addThrowChecker(CCOpenNIUser theUser) {
		ILThrowChecker myThrowChecker = new ILThrowChecker(theUser, CCUserJointType.LEFT_HAND);
		myThrowChecker.events().add(new ILThrowListener() {
	...
	public void update(float theDeltaTime) {
		...
		for(ILThrowChecker myChecker:_myUserMap.values()) {
...
//			myUser.drawVelocities(g,500);
	...
	public static void main(String[] args) {
		CCApplicationManager myManager = new CCApplicationManager(ILUserThrowDemo.class);
