...
public class ILThrowChecker {
	...
	public ILThrowChecker(CCOpenNIUser theUser, CCUserJointType theJointType) {
		_myUser = theUser;
	...
	public void update(float theDeltaTime) {
		...
		if(_myJointType == CCUserJointType.LEFT_HAND) {
			myShoulder = _myUser.joint(CCUserJointType.LEFT_SHOULDER).position().clone();
			myElbow = _myUser.joint(CCUserJointType.LEFT_ELBOW).position().clone();
			myHand = _myUser.joint(CCUserJointType.LEFT_HAND).position().clone();
		}else {
			myShoulder = _myUser.joint(CCUserJointType.RIGHT_SHOULDER).position().clone();
			myElbow = _myUser.joint(CCUserJointType.RIGHT_ELBOW).position().clone();
			myHand = _myUser.joint(CCUserJointType.RIGHT_HAND).position().clone();
		}
